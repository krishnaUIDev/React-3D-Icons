...
import { EuroIconProps } from "./types";

export function EuroIcon(props: EuroIconProps) {
  return (
    <SharedWrapper iconId="euro" {...props}>
      {(mat) => (
        <group rotation={[0.25, -Math.PI / 6, 0.08]} position={[0, 0, 0]} scale={[1.4, 1.4, 1.4]}>
          ...
          {/* Euro Symbol (€ in Center - Glowing Accent) */}
          <group position={[0, 0, 0.026]}>
            {/* Curved C Arc */}
            <mesh position={[0.06, 0, 0]} rotation={[0, 0, Math.PI / 2.5]} castShadow>
              <torusGeometry args={[0.12, 0.018, 8, 16, Math.PI * 1.25]} />

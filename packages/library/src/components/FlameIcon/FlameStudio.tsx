...
import { EmberData, OUTER_PROFILE_POINTS, INNER_PROFILE_POINTS } from "./FlameIcon";

const buildLatheGeometry = (pointsData: number[][]) => {
  const THREE_LIB = window.THREE;
  if (!THREE_LIB) return null;
  const vectors = pointsData.map((p: number[]) => new THREE_LIB.Vector2(p[0], p[1]));
  return new THREE_LIB.LatheGeometry(vectors, 32);
};

// Sub-component for Header
interface HeaderProps {
  applySpectrumPreset: (type: string) => void;
}

function FlameStudioHeader({ applySpectrumPreset }: HeaderProps) {
  return (
...
// Sub-component for Sidebar Controls
interface SidebarProps {
  activeTab: string;
  setActiveTab: (tab: string) => void;
  flatness: number;
  setFlatness: (v: number) => void;
  primaryColor: string;
  setPrimaryColor: (v: string) => void;
  accentColor: string;
  setAccentColor: (v: string) => void;
  coreColor: string;
  setCoreColor: (v: string) => void;
  showLogs: boolean;
  setShowLogs: (v: boolean) => void;
  flameStyle: string;
  setFlameStyle: (v: string) => void;
  lightingPreset: string;
  setLightingPreset: (v: string) => void;
  swayIntensity: number;
  setSwayIntensity: (v: number) => void;
  flickerSpeed: number;
  setFlickerSpeed: (v: number) => void;
  emberCount: number;
  setEmberCount: (v: number) => void;
}

function FlameStudioSidebar({
  activeTab,
  setActiveTab,
  flatness,
  setFlatness,
    ...
    <div className="lg:col-span-5 border-r border-slate-200 bg-slate-100/40 flex flex-col items-center justify-center relative min-h-[450px]">
      <div className="absolute top-4 left-4 z-10 pointer-events-none">
        ...
        <div className="text-sm font-bold text-slate-800 mt-2">
          DYNAMICS FLAME <span className="text-xs font-normal text-slate-500">({flameStyle})</span>
        </div>
...
// Sub-component for Developer Output
interface ExporterProps {
  primaryColor: string;
  downloadPng: () => void;
  codeTab: string;
  setCodeTab: (tab: string) => void;
  getThreeJsCode: () => string;
  copyToClipboard: (text: string, label: string) => void;
}

function FlameStudioExporter({
  primaryColor,
  downloadPng,
  codeTab,
  setCodeTab,
...
function FlameStudioFooter({ flameStyle, orthographic, flatness }: FooterProps) {
  return (
...
// Custom Hook to manage WebGL Engine
function useFlameThreeEngine({
  threeLoaded,
  orthographic,
  primaryColor,
  accentColor,
  ...
  useEffect(() => {
    ...
    const d = 2.4;
    const orthoCamera = new THREE_LIB.OrthographicCamera(-d * aspect, d * aspect, d, -d, 0.1, 100);
    ...
    // Groups hierarchy
    const rotationGroup = new THREE_LIB.Group();
    const flameGroup = new THREE_LIB.Group();
    ...
    // Ambient Lighting
    const ambientLight = new THREE_LIB.AmbientLight(0xffffff, 0.55);
    scene.add(ambientLight);

    // Directional Key Light (gives shiny edges to glass/gold)
    const keyLight = new THREE_LIB.DirectionalLight(0xffffff, 2.0);
    keyLight.position.set(5, 5, 4);
    scene.add(keyLight);
    keyLightRef.current = keyLight;

    // Soft fill
    const fillLight = new THREE_LIB.DirectionalLight(0xffffff, 0.8);
    fillLight.position.set(-5, -2, 2);
    scene.add(fillLight);
    fillLightRef.current = fillLight;

    // Internal Glow PointLight (simulating real flame illumination)
    const coreLight = new THREE_LIB.PointLight(primaryColor, 3, 8);
    coreLight.position.set(0, -0.1, 0);
    scene.add(coreLight);
    coreLightRef.current = coreLight;

    // Pristine Light Grid Floor
    const gridHelper = new THREE_LIB.GridHelper(10, 20, 0x06b6d4, 0xe2e8f0);
    gridHelper.position.y = -1.4;
    scene.add(gridHelper);
    gridHelperRef.current = gridHelper;

    const logsGroup = new THREE_LIB.Group();
    logsGroup.position.y = -0.9;
    flameGroup.add(logsGroup);
    logsGroupRef.current = logsGroup;

    // Dynamic fire log shapes
    const logMaterial = new THREE_LIB.MeshStandardMaterial({
      color: 0x4d2110,
      roughness: 0.9,
      metalness: 0.1
    });
    const logGeom = new THREE_LIB.CylinderGeometry(0.08, 0.1, 1.2, 12);

    const log1 = new THREE_LIB.Mesh(logGeom, logMaterial);
    log1.rotation.set(Math.PI / 2, 0, Math.PI / 4);
    log1.position.set(0, 0, -0.1);
    logsGroup.add(log1);

    const log2 = new THREE_LIB.Mesh(logGeom, logMaterial);
    log2.rotation.set(Math.PI / 2, 0, -Math.PI / 4);
    log2.position.set(0, 0, 0.1);
    logsGroup.add(log2);

    // Glowing charcoal core mesh in middle of wood logs
    const charcoalGeom = new THREE_LIB.SphereGeometry(0.18, 12, 12);
    const charcoalMat = new THREE_LIB.MeshStandardMaterial({
      color: 0xea580c,
      emissive: 0xea580c,
      emissiveIntensity: 1.5
    });
    const charcoal = new THREE_LIB.Mesh(charcoalGeom, charcoalMat);
    charcoal.position.set(0, 0.05, 0);
    logsGroup.add(charcoal);

    const embersGroup = new THREE_LIB.Group();
    flameGroup.add(embersGroup);

    const emberGeom = new THREE_LIB.SphereGeometry(0.025, 6, 6);
    const emberMat = new THREE_LIB.MeshBasicMaterial({
      color: 0xfbbf24,
      transparent: true,
      opacity: 0.9
    });
    ...
    for (let i = 0; i < 30; i++) {
      const mesh = new THREE_LIB.Mesh(emberGeom, emberMat);
    ...
    // Dynamic Materials
    const outerMat = new THREE_LIB.MeshPhysicalMaterial({
      color: primaryColor,
      side: THREE_LIB.DoubleSide,
      transparent: true
    });
    const midMat = new THREE_LIB.MeshStandardMaterial({
      color: accentColor,
      side: THREE_LIB.DoubleSide,
      emissive: accentColor,
      emissiveIntensity: 0.5
    });
    const innerMat = new THREE_LIB.MeshStandardMaterial({
      color: coreColor,
      side: THREE_LIB.DoubleSide,
      emissive: coreColor,
    ...
    // Outer Mesh
    const outerMesh = new THREE_LIB.Mesh(outerGeom, outerMat);
    outerMesh.position.set(0, -0.15, 0);
    flameGroup.add(outerMesh);
    outerFlameRef.current = outerMesh;

    // Mid Flame Mesh
    const midMesh = new THREE_LIB.Mesh(outerGeom, midMat);
    ...
    const innerMesh = new THREE_LIB.Mesh(innerGeom, innerMat);
    ...
    const handleResize = () => {
      ...
      if (orthoCameraRef.current) {
        ...
        const dScale = 2.4;
        orthoCameraRef.current.left = -dScale * activeAspect;
        orthoCameraRef.current.right = dScale * activeAspect;
        orthoCameraRef.current.top = dScale;
        orthoCameraRef.current.bottom = -dScale;
...
// Apply custom designer flatness onto the Mesh scale (X, Y, Z * 0.55)
...
flameMesh.scale.set(1, 1, ${flatness}); // Modern stylized flat icon look!`;
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] text-slate-800 flex flex-col font-sans overflow-x-hidden selection:bg-orange-100 selection:text-orange-900">
      ...
      <FlameStudioHeader applySpectrumPreset={applySpectrumPreset} />
      ...
      <main className="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-0 overflow-hidden">
        ...
        <FlameStudioExporter
          ...
          getThreeJsCode={getThreeJsCode}
          copyToClipboard={copyToClipboard}
        />
      </main>

      <FlameStudioFooter flameStyle={flameStyle} orthographic={orthographic} flatness={flatness} />
    </div>
  );
}

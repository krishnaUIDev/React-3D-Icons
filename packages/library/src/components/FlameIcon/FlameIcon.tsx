...
export function FlameIcon(props: FlameIconProps) {
  const outerPoints = useMemo(() => OUTER_PROFILE_POINTS.map(p => new THREE.Vector2(p[0], p[1])), []);
  const innerPoints = useMemo(() => INNER_PROFILE_POINTS.map(p => new THREE.Vector2(p[0], p[1])), []);
  const flatness = 0.65;
...
function FlameStudio() {
  ...
  useEffect(() => {
    ...
    let script: HTMLScriptElement | null = document.querySelector('script[src*="three.min.js"]') as HTMLScriptElement | null;
    if (!script) {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      const flickerFactor = Math.sin(time * 12 * flickerSpeed) * 0.03 + Math.cos(time * 7 * flickerSpeed) * 0.02;
      ...
      if (midFlameRef.current) {
        ...
        midFlameRef.current.rotation.z = Math.sin(time * 2.8 * flickerSpeed + 1.2) * (swayAmount * 1.1);
        midFlameRef.current.rotation.x = Math.cos(time * 2.1 * flickerSpeed + 0.8) * (swayAmount * 0.8);
      }
      if (innerFlameRef.current) {
        innerFlameRef.current.rotation.z = Math.sin(time * 3.2 * flickerSpeed + 2.4) * (swayAmount * 0.7);
      }
  ...
  useEffect(() => {
    if (!threeLoaded || !outerFlameRef.current || !midFlameRef.current || !innerFlameRef.current) return;
  ...
  const downloadPng = () => {
    if (!rendererRef.current || !sceneRef.current || !activeCameraRef.current || !canvasRef.current) return;
    ...
    <div className="min-h-screen bg-[#f8fafc] text-slate-800 flex flex-col font-sans overflow-x-hidden selection:bg-orange-100 selection:text-orange-900">
      ...
      {toastMessage && (
        <div className="fixed top-6 right-6 z-50 flex items-center bg-white border border-orange-200 text-orange-800 px-4 py-3 rounded-xl shadow-xl backdrop-blur-md animate-bounce">
          <svg className="w-5 h-5 mr-2 stroke-current text-orange-500" fill="none" viewBox="0 0 24 24">
        ...
        <div className="flex items-center gap-3">
          <div className="p-2 bg-gradient-to-br from-orange-400 to-red-500 rounded-xl shadow-md shadow-orange-500/10">
            <svg className="w-7 h-7 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M9.879 16.172a3 3 0 014.242 0M9.879 16.172a3 3 0 01-4.242-4.242L12 6.343l5.657 5.657a3 3 0 01-4.242 4.242L12 12l-2.121 2.172z" />
            </svg>
          </div>
          <div>
            <h1 className="text-xl font-black bg-gradient-to-r from-slate-900 to-slate-700 bg-clip-text text-transparent tracking-tight">FLAMESTUDIO 3D</h1>
            <p className="text-xs text-slate-500 font-medium font-mono">2.5D Premium Flame Component Generator</p>
          </div>
        </div>

        {/* Quick presets */}
        <div className="flex flex-wrap items-center justify-center gap-2 max-w-xl">
          <span className="text-xs font-semibold text-slate-400 mr-1 uppercase tracking-wider hidden lg:inline">Color Presets:</span>
          {[
      ...
      <main className="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-0 overflow-hidden">
          ...
          <div className="p-6 flex-1 flex flex-col gap-6">
            ...
            {activeTab === 'flame' && (
              <div className="space-y-6 animate-fade-in">
                ...
                <div className="bg-white p-5 rounded-xl border border-slate-200 space-y-4 shadow-sm">
                  <div>
                    <span className="block text-xs font-black uppercase text-slate-400 tracking-wider mb-1">2.5D Flattening Offset</span>
                    <span className="block text-[10px] text-slate-400">Compresses Z-axis to create a beautiful flat vector style in isometric space</span>
                  </div>
                  <div className="flex items-center gap-4">
                    ...
                    />
                    <span className="text-xs font-mono font-bold bg-slate-100 px-2.5 py-1 rounded text-slate-600">{flatness}x</span>
                  </div>
                </div>

                {/* Custom Layer Colors */}
                <div className="bg-white p-5 rounded-xl border border-slate-200 space-y-4 shadow-sm">
                  <h4 className="text-xs font-black uppercase text-slate-400 tracking-wider mb-2">Layer Fire Spectrum</h4>

                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      <span className="text-[11px] font-semibold text-slate-500">Outer Envelope</span>
                      <div className="flex items-center gap-1.5">
                        ...
                        />
                        <span className="text-xs font-mono uppercase text-slate-600">{primaryColor}</span>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-1.5">
                        <input
                          ...
                          className="w-8 h-8 rounded-lg cursor-pointer bg-transparent border-0"
                        />
                        <span className="text-xs font-mono uppercase text-slate-600">{accentColor}</span>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-1.5">
                        <input
                          ...
                          className="w-8 h-8 rounded-lg cursor-pointer bg-transparent border-0"
                        />
                        <span className="text-xs font-mono uppercase text-slate-600">{coreColor}</span>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between p-4 rounded-xl border border-slate-200 bg-white shadow-sm">
                  <div>
                    <span className="text-xs font-bold text-slate-700">Stylized Log Fuel Hearth</span>
                    <span className="block text-[10px] text-slate-400 mt-0.5">Appends crossed campfire firewood cylinders at base</span>
                  </div>
                  <button
                  ...
                  >
                    <span className={`inline-block h-4 w-4 transform rounded-full bg-white transition-all ${showLogs ? 'translate-x-6' : 'translate-x-1'} shadow`} />
                  </button>
                </div>
            ...
            {activeTab === 'rendering' && (
              <div className="space-y-6 animate-fade-in">
                ...
                <div>
                  <label className="block text-xs font-bold uppercase text-slate-400 tracking-wider mb-3">Premium Icon Shader</label>
                  <div className="grid grid-cols-2 gap-2">
                    {[
                      ...
                      >
                        <div className={`text-xs font-bold ${flameStyle === style.id ? 'text-orange-900' : 'text-slate-800'}`}>{style.label}</div>
                ...
                <div className="bg-white p-5 rounded-xl border border-slate-200 space-y-4 shadow-sm">
                  <h4 className="text-xs font-bold uppercase text-slate-500 tracking-wider">Studio Environment Rig</h4>
                  <div className="grid grid-cols-3 gap-2">
                    {[
                      ...
                      { id: 'magic', label: 'Magic Nebula', glowClass: 'from-fuchsia-400 to-indigo-400' }
                    ].map((preset) => (
                      <button
                        key={preset.id}
                      ...
                      >
                        <div className={`w-full h-1.5 rounded bg-gradient-to-r ${preset.glowClass} mb-1`} />
                        {preset.label}
                      </button>
                    ))}
                  </div>
            ...
            {activeTab === 'physics' && (
              <div className="space-y-6 animate-fade-in">
                ...
                {/* Wind Sway Rates */}
                <div className="bg-white p-5 rounded-xl border border-slate-200 space-y-4 shadow-sm">
                  <h4 className="text-xs font-bold uppercase text-slate-500 tracking-wider">Dynamic Turbulence Simulator</h4>

                  {/* Sway offset */}
                  <div>
                    <div className="flex justify-between mb-1">
                      <span className="text-[10px] text-slate-400 font-bold uppercase">Turbulence Sway Intensity</span>
                      <span className="text-xs font-mono font-bold text-slate-700">{swayIntensity}x</span>
                    </div>
                    <input
                      type="range"
                      min="0"
                  ...
                  <div>
                    <div className="flex justify-between mb-1">
                      <span className="text-[10px] text-slate-400 font-bold uppercase">Flicker Pulse Frequency</span>
                      <span className="text-xs font-mono font-bold text-slate-700">{flickerSpeed}Hz</span>
                    </div>
                    <input
                      type="range"
                      min="0.2"
                ...
                <div className="bg-white p-5 rounded-xl border border-slate-200 space-y-4 shadow-sm">
                  <div className="flex justify-between mb-1">
                    <div>
                      <span className="text-xs font-bold text-slate-700 uppercase tracking-wider block">Floating Heat Embers</span>
                      <span className="text-[10px] text-slate-400">Total volume of physical spark particles</span>
                    </div>
                    <span className="text-xs font-mono font-bold text-slate-700">{emberCount} sparks</span>
                  </div>
                  <input
                    type="range"
                    min="0"
        ...
        <div className="lg:col-span-5 border-r border-slate-200 bg-slate-100/40 flex flex-col items-center justify-center relative min-h-[450px]">
          ...
          <div className="absolute top-4 left-4 z-10 pointer-events-none">
            <span className="text-[10px] font-black tracking-widest text-orange-600 bg-orange-100 border border-orange-200 px-2 py-1 rounded-md uppercase">VIVID ENGINE 2.5</span>
            <div className="text-sm font-bold text-slate-800 mt-2">DYNAMICS FLAME <span className="text-xs font-normal text-slate-500">({flameStyle})</span></div>
          ...
          >
            {!threeLoaded && (
              <div className="absolute inset-0 flex flex-col items-center justify-center bg-slate-50 z-20 gap-3">
                ...
                <span className="text-xs font-semibold text-slate-500 tracking-wider">Initializing Fluid Engine...</span>
              </div>
            )}
          ...
          <div className="absolute bottom-4 left-4 right-4 z-10 bg-white/95 border border-slate-200 rounded-2xl p-4 shadow-lg backdrop-blur-md flex flex-wrap items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-4">
              <div className="flex items-center gap-2">
                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Show Helper Floor:</span>
                <button
            ...
            <div className="flex items-center gap-2">
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Projection:</span>
              <button
        ...
        <div className="lg:col-span-3 bg-slate-50 flex flex-col overflow-y-auto max-h-[calc(100vh-73px)] border-l lg:border-l-0 border-slate-200">
          ...
          <div className="border-b border-slate-200 bg-white p-4 sticky top-0 z-10">
            <h3 className="text-xs font-black uppercase text-slate-500 tracking-wider">Component Export</h3>
            <p className="text-[10px] text-slate-400 mt-0.5">Capture transparent PNG assets or fetch code</p>
          </div>
          ...
          <div className="p-6 space-y-6 flex-1 flex flex-col">
            ...
            <div className="space-y-3">
              <span className="block text-xs font-bold uppercase text-slate-400 tracking-wider">Interactive UI Presentation (Card CSS)</span>
              ...
              >
                ...
                <div className="w-20 h-20 rounded-2xl flex items-center justify-center border border-slate-100 shadow-inner mb-4 relative" style={{ backgroundColor: '#fffbf9' }}>
                  <span className="w-8 h-10 rounded-full animate-pulse transition-colors" style={{ backgroundColor: primaryColor, clipPath: 'polygon(50% 0%, 100% 38%, 82% 100%, 18% 100%, 0% 38%)' }} />
                </div>

                <h4 className="text-sm font-extrabold text-slate-800 tracking-tight">Active Flame Item</h4>
            ...
            {/* Downloader action */}
            <div className="space-y-3">
              <span className="block text-xs font-bold uppercase text-slate-400 tracking-wider">Assets Exporter</span>
              <button
                onClick={downloadPng}
            ...
            <div className="flex-1 flex flex-col min-h-[220px]">
              ...
              <div className="flex-1 bg-slate-900 rounded-xl border border-slate-800 relative group max-h-[180px] overflow-y-auto">
                ...
                <button
                  onClick={() => copyToClipboard(getThreeJsCode(), 'ThreeJS profile geometry source')}
      ...
      <footer className="border-t border-slate-200 bg-white px-6 py-4 flex flex-col sm:flex-row items-center justify-between text-xs text-slate-400 gap-4">
        <div className="flex items-center gap-6">
          <span>Active Shader: <strong className="text-slate-700 font-semibold">{flameStyle.toUpperCase()}</strong></span>
          <span>Camera aspect ratio: <strong className="text-slate-700 font-semibold">{orthographic ? 'Orthogonal 2D' : 'Perspective 3D'}</strong></span>
          <span>Flattening Offset: <strong className="text-slate-700 font-semibold">{flatness}x</strong></span>
        </div>

...
import { FolderPlusIconProps } from "./types";

export function FolderPlusIcon(props: FolderPlusIconProps) {
  return (
    <SharedWrapper iconId="folderplus" {...props}>
      {(mat) => (
        <group rotation={[0.22, -Math.PI / 6, 0.05]} position={[0, 0, 0]} scale={[1.4, 1.4, 1.4]}>
          ...
          {/* Raised 3D Plus Symbol on Bottom Right Front Cover (Emissive Accent Color) */}
          <group position={[0.1, -0.05, 0.046]}>
            {/* Horizontal Bar */}
            <mesh castShadow>

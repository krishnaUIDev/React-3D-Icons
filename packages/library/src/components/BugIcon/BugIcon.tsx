      ...
      {(mat) => {
        ...
        return (
          <group rotation={[0.1, -0.25, 0.05]}>
            ...
            {/* Antennae */}
            {([[-0.14, 0.88, 0.07, -0.35], [0.14, 0.88, 0.07, 0.35]] as number[][]).map(([x, y, z, rx], i) => (
            ...
            {/* Legs (3 per side) */}
            {([-1, 0, 1] as number[]).map((row, i) => ([-1, 1] as number[]).map((side, j) => (
              <mesh key={`${i}-${j}`} position={[side * 0.55, row * 0.22 - 0.05, 0]} rotation={[0, 0, side * 0.9]}>
